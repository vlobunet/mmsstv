...
void __fastcall TMmsstv::WndCopyData(TMessage &Message)
{
	...
	switch(cp->dwData){
		...
		case 1:             // Hamlog‚©‚з‚М•ФђM
			...
			Message.Result = TRUE;
			break;
		case 0x80001212:    // Ћь”gђ”ѓfЃ[ѓ^‚МЋw’и
			...
			Message.Result = TRUE;
			break;
		case 0x80001213:    // ѓNѓЉѓbѓvѓ{Ѓ[ѓh‚©‚з‚МѓЌЃ[ѓh
			KESClick(NULL);
			Message.Result = TRUE;
			break;
		case 0x80001214:    // ‰ж‘њѓTѓCѓY‚М–в‚ўЌ‡‚н‚№
			Message.Result = m_TXW + (m_TXH << 16);
			break;
		case 0x80001215:    // —љ—р‰ж‘њ‚МѓRѓsЃ[
			SBCopyClick(NULL);
			Message.Result = TRUE;
			break;
		case 0x80001216:    // ЋуђM‰ж‘њ‚МѓRѓsЃ[
			KERClick(NULL);
			Message.Result = TRUE;
			break;
		case 0x80001217:    // ‘—ђM/ЋуђM‚МђШ‚и‘Ц‚¦
			if( cp->cbData && (cp->lpData != NULL) ){
		...
		case 0x80001218:
			...
			Message.Result = TRUE;
			break;
		case 0x80001219:    // REMѓfЃ[ѓ^‚МЋw’и
			if( cp->cbData && (cp->lpData != NULL) ){
...
if (dwVersion < 0x80000000)                // Windows NT
	...
	if( lcid != LANG_JAPANESE ){                    // English
		...
		sys.m_WinFontName = "‚l‚r ‚oѓSѓVѓbѓN";
...
void __fastcall TMmsstv::UpdateToneFreq(void)
{
	...
	char bf[128];
	sprintf(bf, MsgEng ? "Send %uHz tone" : "%uHzД°Э‚М‘—ђM", fq);
	SBTune->Hint = bf;
	SBTune->Caption = fq;
}
//---------------------------------------------------------------------------
...
// ѓЉѓsЃ[ѓ^
		...
		ErrorMB(MsgEng?"Cannot update MMSSTV.INI":"MMSSTV.INI‚рЌXђV‚Е‚«‚Ь‚№‚с.");
	}
...
void __fastcall TMmsstv::FormPaint(TObject *Sender)
{
	if( m_InitFirst ){
		...
		if( (sys.m_Call == "NOCALL") || sys.m_Call.IsEmpty() ){
			...
			if( InputMB("MMSSTV", MsgEng ? "Please enter your callsign":"‚ ‚И‚Ѕ‚МѓRЃ[ѓ‹ѓTѓCѓ“‚р“ь—Н‚µ‚Д‰є‚і‚ў.", as) == TRUE ){
				if( !as.IsEmpty() ){
					char bf[MLCALL+1];
...
//---------------------------------------------------------------------------
// Њ»ЌЭ‚М_—ќѓpѓЊѓbѓg‚р•Ф‚·ЃiTControl::GetPalette‚МѓIЃ[ѓoѓ‰ѓCѓhЉЦђ”Ѓj
...
//---------------------------------------------------------------------------
// ѓXѓЊѓbѓh“а‚ЕѓRЃ[ѓ‹‚µ‚Д‚Н‚ў‚Ї‚И‚ў
...
//                ::SetWindowPos(pRxView->Handle, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE	);
		...
		if( pDem->m_fskrec ){
			pDem->m_fskrec = 0;
			if( pDem->m_Sync && ((pCtrBtn == NULL) || (!pCtrBtn->SBLock->Down)) ){     // ‹­ђ§’вЋ~
...
{
	if( (sys.m_AutoStop || sys.m_AutoSync || KRSA->Checked) && (SSTVSET.m_Mode != smAVT) ){   // Ћ©“®’вЋ~‚Мѓ`ѓFѓbѓN
...
void __fastcall TMmsstv::InfoRxLost(void)
{
	InfoMB( MsgEng ?
		...
		"ЋуђM’†‚ЙCPU‰Я•‰‰Ч‚Й‚ж‚иѓTѓEѓ“ѓh‚р€к•”Њ‡—Ћ‚µ‚Ь‚µ‚Ѕ.\r\n\r\n"
		"ђЭ’и‰ж–К‚МЃu‚»‚М‘јЃvѓyЃ[ѓW‚МѓTѓEѓ“ѓhѓJЃ[ѓh‚МFIFO‚р‘ќ‚в‚µ‚Д‰є‚і‚ў"
	);
}
void __fastcall TMmsstv::InfoTxLost(void)
{
	InfoMB( MsgEng ?
		...
		"Do increase FIFO on sound in the Misc page of Setup MMSSTV." :
		"‘—ђM’†‚ЙCPU‰Я•‰‰Ч‚Й‚ж‚иѓTѓEѓ“ѓh‚р€к•”Њ‡—Ћ‚µ‚Ь‚µ‚Ѕ.\r\n\r\n"
		"ђЭ’и‰ж–К‚МЃu‚»‚М‘јЃvѓyЃ[ѓW‚МѓTѓEѓ“ѓhѓJЃ[ѓh‚МFIFO‚р‘ќ‚в‚µ‚Д‰є‚і‚ў"
	);
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::KFRecClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "MMSSTVѓTѓEѓ“ѓhѓtѓ@ѓCѓ‹‚МЌмђ¬";
		SaveDialog->Filter = "MMSSTVѓTѓEѓ“ѓhѓtѓ@ѓCѓ‹(*.mmv)|*.mmv|";
	}
...
void __fastcall TMmsstv::KFPlayClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "MMSSTVѓTѓEѓ“ѓhѓtѓ@ѓCѓ‹‚МЌДђ¶";
		OpenDialog->Filter = "MMSSTVѓTѓEѓ“ѓhѓtѓ@ѓCѓ‹(*.mmv)|*.mmv|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TMmsstv::CorrectSlant(void)
{
	...
	if( ((dp->m_StgBuf != NULL) || WaveStg.IsOpen()) && (dp->m_wStgLine >= 16) && (SSTVSET.m_Mode != smAVT) ){
		...
		int LW = SSTVSET.m_TW * 0.1;       // 10%‚М—h‚к‚р‹–—e
		int z;
		for( z = 0; z < 5; z++ ){
			// ЉоЏЂ€К’u‚р’T‚·
			WaveStg.RInit();
			...
			for( i = 0; i < dp->m_wStgLine; i++ ){
				...
				for( j = 0; j < SSTVSET.m_WD; j++, sp++, base++ ){
					...
					if( yy != y ){
						if( bpos < 0 ){
							if( ps >= (SSTVSET.m_TW/4) ){         // Ќ¶•ыЊь‚Ц‚МЋь‚и‚±‚Э
								ps -= SSTVSET.m_TW;
							}
						...
						else if( bpos >= SSTVSET.m_TW ){        // ‰E•ыЊь‚Ц‚МЋь‚и‚±‚Э
						...
						else if( bpos >= (SSTVSET.m_TW*3/4) ){  // ‰E‘¤
							if( (ps < SSTVSET.m_TW/4) ){
								ps += SSTVSET.m_TW;
							}
						}
						else if( bpos <= (SSTVSET.m_TW/4) ){    // Ќ¶‘¤
							if( ps >= (SSTVSET.m_TW*3/4) ){
								ps -= SSTVSET.m_TW;
							}
						}
...
int __fastcall TMmsstv::AdjustSyncPos(int n)
{
		...
		switch(SSTVSET.m_Mode){
			...
			case smSCTDX:
				...
				break;
			case smMRT1:        // Њґ€ц•s–ѕ‚Мѓ^ѓCѓ~ѓ“ѓOѓYѓЊ‚р•вђі
			case smSC2_180:
			case smSC2_120:
			case smP3:
			case smP5:
...
{
	if( pSound == NULL ) return;
	if( m_ReqSampChg ){             // ѓTѓ“ѓvѓЉѓ“ѓOЋь”gђ”•ПЌX—v‹Ѓ’†
		m_ReqSampChg = 0;
		RedrawSampFreq(FALSE);
		return;
	}
...
{
#if 1
	// Tapђ”‚р‘ќ‚в‚µ‚ЅЏкЌ‡‚МЏ€—ќ
    // 2003/11/28‚ЙЋАЊ±‚µЃAЊ‹‰КTapђ”‚НЏ]—€’К‚и‚М‚S‚Й–Я‚µ‚Ѕ
    // ‚М‚Е‚±‚МЏ€—ќ‚рѓLѓѓѓ“ѓZѓ‹‚µ‚Ѕ
    int d = GetLMSDelay();
...
{
	if( UDHist->Position < RxHist.m_Head.m_Cnt ){
		if( YesNoMB( MsgEng ? "Delete the picture, are you sure?" : "‚Щ‚с‚Ь‚ЙЏБ‚µ‚Ь‚·‚©ЃH" ) == IDYES ){
...
void __fastcall TMmsstv::KHDAClick(TObject *Sender)
{
	if( RxHist.m_Head.m_Cnt ){
		if( YesNoMB( MsgEng ? "Delete all pictures, are you sure?" : "‘S‚Д‚М—љ—р‚рЌнЏњ‚µ‚Ь‚·." ) == IDYES ){
			RxHist.DeleteAll();
			UDHist->Position = 0;
			UpdateHist();
	...
	if( IsSR() ){
		if( mp->m_wLine >= SSTVSET.m_TL ){
			if( mp->m_wLine == SSTVSET.m_TL ){
				...
				int lost = mp->m_Lost;
//                ::Sleep( (pSound->m_BuffSize * pSound->m_FifoSize * 1000) / SSTVSET.m_TxSampFreq ); // ѓoѓbѓtѓ@‘—ђMЉ®—№‘Т‚ї
				ToRX();
				if( lost ) InfoTxLost();
				return;
			}
...
void __fastcall TMmsstv::ToTX(void)
{
	...
	if( SBUseTemp->Down && DrawMain.m_Cnt && (pBitmapTXM->Width != DrawMain.m_SX) && (!rep) ){
		...
		if(
			...
			"ѓeѓ“ѓvѓЊЃ[ѓg‚МѓTѓCѓY‚Є‘—ђM‰ж‘њ‚Ж€Щ‚И‚й‚Ѕ‚Яѓeѓ“ѓvѓЊЃ[ѓg‚М‰жЋї‚Н—З‚­‚ ‚и‚Ь‚№‚с.\r\n\r\n‚±‚М‚Ь‚Ь‘—ђM‚µ‚Ь‚·‚©ЃH" ) == IDNO
		){
			return;
		}
		SBTX->Down = TRUE;
	...
	OutHEAD();
	// VIS
	if( SSTVSET.m_fTxNarrow ){			// ‹·‘С€жѓ‚Ѓ[ѓh‚М‘—ђM
		mp->Write(1900, 300);
		mp->Write(FSKSPACE, FSKGARD);
		mp->Write(1900, FSKINTVAL);
...
//		mp->Write(FSKSPACE, FSKGARD);
		...
		if( SSTVSET.m_TxMode == smAVT ){       // “ЇЉъђMЌ†
			int sd = 0x5fa0;
			for( i = 0; i < 32; i++ ){
...
_n1:;
	...
	if( SBUseTemp->Down || (m_MainPage == pgTemp) ){
		...
		if( rep && sys.m_RepBottomAdj ){    // ђLЏk‚И‚µ
...
void __fastcall TMmsstv::InfoSampFreq(void)
{
	if( fabs(sys.m_SampFreq - SampFreq) >= 50.0 ){
		...
		InfoMB(
			...
			"ђV‚µ‚ўѓTѓ“ѓvѓЉѓ“ѓOѓЊЃ[ѓg‚р“K—p‚µ‚Ь‚·ЃBMMSSTV‚рЌД‹N“®‚µ‚Д‰є‚і‚ў\r\n"
			"(ѓpѓ\ѓRѓ“‚рЌД‹N“®‚·‚й•K—v‚Н‚ ‚и‚Ь‚№‚с)\r\n\r\n"
			"Њ»ЌЭMMSSTV‚Н%.2lfHz‚Е“®Ќм‚µ‚Д‚Ё‚иЃA%.2lfHz‚Ц‚М•ПЌX‚р—vђї‚і‚к‚Д‚ў‚Ь‚·ЃB"
			"  ЌД‹N“®‚µ‚И‚ў‚ЖђіЏн‚И“®Ќм‚р€ЫЋќ‚·‚й‚±‚Ж‚Є‚Е‚«‚Ь‚№‚сЃB"
...
//        if( !sys.m_TxRxLock ) pComm->Close();
	...
	if( strcmp(RADIO.StrPort, "NONE" ) ){
		...
		if( pRadio->m_CreateON != TRUE ){
			delete pRadio;
			pRadio = NULL;
			ErrorMB( MsgEng ? "Could not open '%s' for Radio commands":"RadioѓRѓ}ѓ“ѓhѓ|Ѓ[ѓg'%s'‚ЄѓIЃ[ѓvѓ“‚Е‚«‚Ь‚№‚с.", RADIO.StrPort);
		}
		else {
...
void __fastcall TMmsstv::FindCall(void)
{
	...
	if( Log.FindSet(&Log.m_Find, Log.m_sd.call) ){  // Њ©‚В‚©‚Б‚ЅЋћ
		MultProc();
		Log.SetFreq(&Log.m_sd, AnsiString(LogFreq->Text).c_str());	//ja7ude 0428
		SDMMLOG	sd;
		Log.GetData(&sd, Log.m_Find.pFindTbl[0]);	// €к”ФЌЕђV‚МѓfЃ[ѓ^
...
void __fastcall TMmsstv::KLogOpenClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "ѓЌѓOѓtѓ@ѓCѓ‹‚МѓIЃ[ѓvѓ“";
		OpenDialog->Filter = "MMLOGѓfЃ[ѓ^ѓtѓ@ѓCѓ‹(*.mdt)|*.mdt|";
	}
	OpenDialog->FileName = "";
	  ...
	  TShiftState Shift, int X, int Y)
{
	...
	if( sc1 == sc2 ){			// Left button
		AdjustTempView(1);
		if( pDraw != NULL ){                // ђV‹KЌмђ¬’†
			pDraw->Making(X, Y);
		}
		else if( DrawMain.pSel != NULL ){   // €Ъ“®/ѓTѓCѓY•ПЌX’†
			if( !m_TempTimer || (ABS(X-m_TempX)+ABS(Y-m_TempY)) > 5 ){
				m_TempTimer =0;
				DrawMain.pSel->Moving(X, Y);
			}
		}
		else if( !PBoxTemp->Dragging() ){   // ѓhѓ‰ѓbѓO
			//PBoxTemp->BeginDrag(TRUE);
...
int __fastcall TMmsstv::LoadTemplateMenu(CDrawGroup *pItem, int isw)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "ѓeѓ“ѓvѓЊЃ[ѓgѓtѓ@ѓCѓ‹‚МѓЌЃ[ѓh";
	}
	if( isw ){
		char bf[256];
...
int __fastcall TMmsstv::SaveTemplateMenu(CDrawGroup *pItem, LPCSTR pName, LPCSTR pFolder, int isw)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "ѓeѓ“ѓvѓЊЃ[ѓg‚рѓZЃ[ѓu";
	}
	if( isw ){
...
void __fastcall TMmsstv::PopupSPopup(TObject *Sender)
{
	...
	else if( TabS->TabIndex ){   // ѓeѓ“ѓvѓЊЃ[ѓg
		KSC->Enabled = FALSE;
		KSP->Enabled = FALSE;
		KSFJ->Enabled = FALSE;
		KSFB->Enabled = FALSE;
...
void __fastcall TMmsstv::PopupTPopup(TObject *Sender)
{
	...
	if( cmd != CM_PIC ){
		KTFX->Caption = MsgEng ? "Convert to image item" : "‰ж‘њѓAѓCѓeѓЂ‚Й•ПЉ·";
	}
	else {
		KTFX->Caption = MsgEng ? "Convert to bitmap" : "ѓrѓbѓgѓ}ѓbѓv‚Й•ПЉ·";
	}
...
//                            Graphics::TBitmap *pBitmap = LoadBitmapS(m_DragNo);
	...
	else if( Msg.hwnd == PanelTX->Handle ){     // ‘—ђM‰ж–К‚Ц‚МѓhѓЌѓbѓv
		if( ::DragQueryFile(hDrop, 0, fname, sizeof(fname)) ){
			::SetForegroundWindow(Handle);
			LPCSTR pExt = GetEXT(fname);
			if( IsPic(pExt) ){
...
//                ErrorMB("'%s' is not supported.", fname);
	...
	else if( Msg.hwnd == PanelTemp->Handle ){       // ѓeѓ“ѓvѓЊЃ[ѓg‚Ц‚МѓhѓЌѓbѓv
_tdrop:;
		if( ::DragQueryPoint(hDrop, &N) ){
			if( ::DragQueryFile(hDrop, 0, fname, sizeof(fname)) ){
				::SetForegroundWindow(Handle);
...
void __fastcall TMmsstv::KHCDClick(TObject *Sender)
{
	if( MsgEng ){
		...
		SaveDialog->Title = "—љ—рѓtѓ@ѓCѓ‹‚МѓtѓHѓ‹ѓ_‚р•ПЌX";
	}
	SaveDialog->Filter = "";
...
void __fastcall TMmsstv::KSCDClick(TObject *Sender)
{
	if( MsgEng ){
		...
		SaveDialog->Title = "ѓXѓgѓbѓNѓtѓ@ѓCѓ‹‚МѓtѓHѓ‹ѓ_‚р•ПЌX";
	}
	SaveDialog->Filter = "";
	  ...
	  TMouseButton Button, TShiftState Shift, int X, int Y)
{
	if( Button == mbRight ){
		...
		if( InputMB("MMSSTV", MsgEng?"Enter tone frequency (Hz)":"ѓgЃ[ѓ“Ћь”gђ”(Hz)‚р“ь—Н", as) == TRUE ){
			int d;
			sscanf(as.c_str(), "%u", &d);
			if( (d >= 100) && (d <= 3000) ){
				pMod->m_TuneFreq = d;
...
void __fastcall TMmsstv::KHFJClick(TObject *Sender)
{
	...
	int r = IDYES;
	if( RxHist.m_Head.m_Cnt ){
		r = YesNoMB( MsgEng ? "MMSSTV will convert the format in all the files which were stored already.\r\n\rThis processing may take a long time, are you sure?" : "Љщ‘¶‚Мѓtѓ@ѓCѓ‹‚МѓtѓHЃ[ѓ}ѓbѓg‚р•ПЌX‚µ‚Ь‚·.\r\n\r\n‚±‚МЏ€—ќ‚НЋћЉФ‚Є‚©‚©‚й‰В”\ђ«‚Є‚ ‚и‚Ь‚·. ‚Щ‚с‚Ь‚Й•ПЉ·‚µ‚Ь‚·‚©ЃH" );
	}
	if( r == IDYES ){
...
void __fastcall TMmsstv::KHACDClick(TObject *Sender)
{
	...
	if( InputMB("MMSSTV", MsgEng ?
		...
		"ѓRѓsЃ[ђж‚МѓtѓHѓ‹ѓ_–ј‚р“ь—Н‚µ‚Д‰є‚і‚ў.", as) == TRUE
	){
		jstrupr(as.c_str());
...
void __fastcall TMmsstv::InitProfile(void)
{
    ...
    mp = m_DemPro;
	mp->Name = MsgEng ? "Hilbert transform" : "ѓqѓ‹ѓxѓ‹ѓg•ПЉ·";
	mp->Type = 2;

    mp++;
	mp->Name = MsgEng ? "Zero crossing" : "ѓ[ѓЌѓNѓЌѓXЊџ”g";
	mp->Type = 1;
	mp->crossOutOrder = 3;
	mp->crossOutFC = 900;
	mp->DemCalibration = 1;
    ...
    mp++;
	mp->Name = MsgEng ? "Zero crossing with Differentiator" : "ѓ[ѓЌѓNѓЌѓXЊџ”g+”ч•ЄЏ€—ќ";
	mp->Type = 1;
	mp->crossOutOrder = 3;
	mp->crossOutFC = 900;
	mp->Differentiator = 1;
...
void __fastcall TMmsstv::KPAClick(TObject *Sender)
{
	...
	for( i = 0; i < 8; i++ ){
		...
		if( Sender == tp ){
			...
			if( InputMB(MsgEng ? "Assign profile" : "ѓvѓЌѓtѓ@ѓCѓ‹‚М“o^", (Font->Charset != SHIFTJIS_CHARSET) ? "Enter name" : "–ј‘O‚р“ь—Н", as) == TRUE ){
				m_DemPro[i].Name = as;
...
void __fastcall TMmsstv::Repeater(void)
{
	...
	if( sys.m_RepBeacon ){
		...
		if( dp->m_RepSQ && (dp->m_repsig > dp->m_RepSQ) ){    // ђMЌ†ЊџЏo’†
			if( CTick < (NTick + sys.m_RepQuietnessTime*1000) ){ // 300s‘O‚МЋћ‚Н‰„’·
...
void __fastcall TMmsstv::SetBeaconDelay(void)
{
	...
	if( CTick < (NTick + qtime*1000) ){ // 300s‘O‚МЋћ‚Н‰„’·
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
	if( Button == mbRight ){
		...
		if( InputMB("MMSSTV", MsgEng ? "Enter text" : "ѓeѓLѓXѓg‚р“ь—Н", as, TRUE) == TRUE ){
			sys.m_CWText = as;
		}
#endif
    }
...
{
	AnsiString as = sys.m_CWText;
	if( InputMB(MsgEng ? "CW button" : "CWѓ{ѓ^ѓ“", MsgEng ? "Enter text" : "ѓeѓLѓXѓg‚р“ь—Н", as, TRUE) == TRUE ){
		sys.m_CWText = as;
	}
}
//---------------------------------------------------------------------------
void __fastcall TMmsstv::PopupCWPopup(TObject *Sender)
{
    ...
    AnsiString as = MsgEng ? "Set button text... (" : "ѓ{ѓ^ѓ“‚М‘—ђMѓeѓLѓXѓg‚МђЭ’и... (";
    as += bf;
    as += ")";
    KCW->Caption = as;
	...
	as += MsgEng ? "Enter PARIS speed..." : "PARIS‘¬“x‚Е“ь—Н...";
	KCWSI->Caption = as;
}
//---------------------------------------------------------------------------
void __fastcall TMmsstv::KCWADDClick(TObject *Sender)
{
	...
	AnsiString as;
	if( InputMB(MsgEng ? "Add CW menu" : "CWѓЃѓjѓ…Ѓ[’З‰Б", MsgEng ? "Enter text" : "ѓeѓLѓXѓg‚р“ь—Н", as, TRUE) == TRUE ){
		if( !as.IsEmpty() ){
...
void __fastcall TMmsstv::KCWEditClick(TObject *Sender)
{
	...
	if( (n >= 0) && (n < sys.m_nCWMenu) ){
		...
		if( InputMB(MsgEng ? "Edit CW menu" : "CWѓЃѓjѓ…Ѓ[•ТЏW", MsgEng ? "Enter text (NULL for delete)" : "ѓeѓLѓXѓg‚р“ь—Н(NULL‚ЕѓЃѓjѓ…Ѓ[‚рЌнЏњ)", as, TRUE) == TRUE ){
			if( as.IsEmpty() ){
...
void __fastcall TMmsstv::KRLoadClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		pBox->Title = "ѓRѓ}ѓ“ѓh’и‹`ѓtѓ@ѓCѓ‹‚рЉJ‚­";
		pBox->Filter = "ѓRѓ}ѓ“ѓh’и‹`ѓtѓ@ѓCѓ‹(*.rcm)|*.rcm|";
	}
	pBox->FileName = "";
	pBox->DefaultExt = "rcm";
