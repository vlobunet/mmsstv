...
void __fastcall TPicRectDlg::UpdateTitle(void)
{
	...
	sprintf(bf, "%s - Source:%ux%u,  Target:%ux%u", MsgEng ? "Picture clipper" : "‰ж‘њѓNѓЉѓbѓpЃ[", pBitmap->Width, pBitmap->Height, pwBmp->Width, h);
	Caption = bf;
}
//---------------------------------------------------------------------
...
void __fastcall TPicRectDlg::PBoxPaint(TObject *Sender)
{
    ...
    if( m_RotFlag ){
		...
		PBox->Canvas->TextOut(0, 0, MsgEng ? "[Rotation] Draw a base line" : "[‰с“]] ѓxЃ[ѓXѓ‰ѓCѓ“‚р€ш‚ў‚Д‚­‚ѕ‚і‚ў");
    }
}
//---------------------------------------------------------------------------
...
{
	AnsiString as;
	if( InputMB(NULL, MsgEng ? "Enter rotation degree" : "‰с“]Љp(Ѓ‹)‚р“ь—Н", as) == IDOK ){
		double deg;
