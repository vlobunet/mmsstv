...
#pragma option -a	// ѓpѓbѓN‰рЏњ
class CThumb {
public:
	int             m_UpdateBmp;    // ѓAѓbѓvѓfЃ[ѓgѓtѓ‰ѓO
	int             m_UpdateIdx;
	int             m_FolderIndex;
	int             m_FileCount;
	int             m_Top;
	...
	int             m_SizeY;
	int             m_XW;
	DIND            *pITBL;         // ѓCѓ“ѓfѓbѓNѓX‚МѓeЃ[ѓuѓ‹
	Graphics::TBitmap *pBitmap;     // ѓrѓbѓgѓ}ѓbѓv
public:
	__fastcall CThumb();
	__fastcall ~CThumb(){
		CloseFolder();
...
public:
	int         m_Type;
	AnsiString  m_Folder;                   // ѓtѓHѓ‹ѓ_–ј
	AnsiString  m_Name;                     // ѓyЃ[ѓW‚М–ј‘O
	int         m_Size[AHDMAX];             // ‰ж‘њѓTѓCѓY‚М”z—с
	int         m_CurPage;                  // Њ»ЌЭ‚МѓyЃ[ѓW”ФЌ†
	int         m_MaxPage;                  // ЌЕ‘еѓyЃ[ѓWђ”
	int         m_UseIndex;                 // ѓCѓ“ѓfѓbѓNѓX‚рЋg‚¤
	CFILEL      *pList;                     // ѓtѓ@ѓCѓ‹–ј‚МѓЉѓXѓg
	Graphics::TBitmap *pBitmapS;            // Col x Line‚Мѓrѓbѓgѓ}ѓbѓv
	CThumb      m_Thumb;
public:
	__fastcall CFILEV();
...
private:	// ѓ†Ѓ[ѓUЃ[ђйЊѕ
	...
	int     m_Max;          // •\Ћ¦‰В”\‚ИѓTѓЂѓlѓCѓ‹‚Мђ”
	int     m_Col;          // •\Ћ¦‰В”\‚ИѓTѓЂѓlѓCѓ‹‚МѓJѓ‰ѓЂђ”
	int     m_Line;         // •\Ћ¦‰В”\‚ИѓTѓЂѓlѓCѓ‹‚МЌsђ”
	int     m_DisEvent;
...
public:
	int     m_MyIndex;
	int     m_Overlap;      // ѓEѓCѓ“ѓhѓE‚МЏd‚И‚и‚р’І‚Ч‚й
	int     m_Suspend;      // •\Ћ¦‚М€кЋћ“I‚И’вЋ~
	int     m_TitleBar;
	...
	int     m_SSize;        // ѓTѓЂѓlѓCѓ‹‚МЊ`ЏуѓTѓCѓY
