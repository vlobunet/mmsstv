...
/*
1.12 JASTAѓRѓ“ѓeѓXѓgЃ@Џd•Ўѓ`ѓFѓbѓN‚МЏCђі
*/
#define VERBETA "A"
...
#define FPAGEMAX    10              // ѓtѓHѓ‹ѓ_‚МѓTѓЂѓlѓCѓ‹‚МѓyЃ[ѓWђ”
#define	CWMENUMAX	128

#define HISTNAME    "History.bin"
#define TEMPV105    "CurTemp.bin"   // V1.05‚Ь‚Е‚М–ј‘O
#define TEMPV105B   "CurTemp.old"
#define TEMPV106C   "Current.mtm"   // V1.06‚©‚з‚М–ј‘O
#define TEMPV106L   "List.mtm"      // V1.06‚©‚з‚М–ј‘O
//#define SHIFTL	10
//#define	MARKL	300
//#define	SPACEH	2700
...
extern	LCID	lcid;				// ѓЌѓPЃ[ѓ‹Џо•с
extern	int		DisPaint;
extern  int     MsgEng;
extern  int     WinNT;
extern	int		WinVista;

extern	double	SampFreq;			// ѓTѓ“ѓvѓЉѓ“ѓOЋь”gђ”
...
extern	char	RecDir[256];		// ^‰№ѓtѓ@ѓCѓ‹‚МѓfѓBѓЊѓNѓgѓЉ
...
typedef struct {
	...
	int         m_Repeater;
	int         m_RepSenseLvl;  // ѓgЃ[ѓ“ЊџЏoЉґ“x
	AnsiString  m_RepAnsCW;
	int         m_RepTimeA;     // ѓgЃ[ѓ“ЊџЏoЋћЉФ
	int         m_RepTimeB;     // ѓgЃ[ѓ“ЊџЏo‚©‚зAnsCWЏo—Н‚Ь‚Е‚МЋћЉФ
	int         m_RepTimeC;     // ЋуђM‘Т‹@‚Мѓ^ѓCѓЂѓAѓEѓg
	int         m_RepTimeD;     // ѓЉѓvѓЊѓC‘—ђM‚М’x‰„ЋћЉФ
...
///---------------------------------------------------------
///  ѓRѓ“ѓgѓЌЃ[ѓ‹‚МѓAѓ‰ѓCѓ“‚МЉЗ—ќѓNѓ‰ѓX
class CAlign
{
private:
	int		BTop, BLeft;
...
///-------------------------------------------------------
/// CRecentMenuѓNѓ‰ѓX
#define	RECMENUMAX	4	// Џ€—ќ‚Е‚«‚йЌЕ‘е‚Мђ”
class CRecentMenu
{
private:
public:
	int		InsPos;		// ѓЃѓjѓ…Ѓ[‘}“ь€К’u‚М”ФЌ†
	int		Max;		// Џ€—ќ‚·‚йђ”
...
private:
	...
	DWORD	m_dwCount;
	DWORD	m_dwID;		// ID’l
	UINT	m_uMsg;		// ѓЃѓbѓZЃ[ѓW’l
	HWND	m_hWndMsg;	// ѓЃѓbѓZЃ[ѓW‚М‘—ђMђж
